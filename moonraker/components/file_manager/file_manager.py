...
METADATA_SCRIPT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "metadata.py"),
)
WATCH_FLAGS = (
    iFlags.CREATE
    | iFlags.DELETE
...
class FileManager:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        self.fs_observer = obs_class(config, self, self.gcode_metadata, self.sync_lock)
        ...
        self.server.register_endpoint(
            ...
            RequestType.POST,
            self._handle_metascan_request,
        )
        self.server.register_endpoint(
            "/server/files/thumbnails",
            RequestType.GET,
            self._handle_list_thumbs,
        )
        self.server.register_endpoint(
            "/server/files/roots",
            RequestType.GET,
            self._handle_list_roots,
        )
        self.server.register_endpoint(
    ...
    def _update_fixed_paths(self) -> None:
        ...
        paths: dict[str, Any] = {
            k: kinfo.get(k)
            for k in ["klipper_path", "python_path", "log_file", "config_file"]
        }
        ...
        klipper_path = paths.get("klipper_path")
        ...
        if log_file is not None:
            ...
            self.server.register_static_file_handler("klippy.log", log_path, force=True)
        ...
        if self.check_klipper_path:
            ...
            if cfg_file is not None and cfg_parent is not None:
                ...
                if (
                    par_path in cfg_path.parents
                    or par_path.resolve() in cfg_path.resolve().parents
                ):
    ...
    def check_write_enabled(self):
        if not self.full_access_roots:
            raise self.server.error(
                "Write access is currently disabled.  Check notifications for warnings.",
            )
    ...
    def register_directory(
        ...
        if full_access:
            if os.access in os.supports_effective_ids and not os.access(
                path,
                os.W_OK,
                effective_ids=True,
            ):
    ...
    def check_reserved_path(
        ...
        if ".git" in req_path.parts:
            if raise_error:
                raise self.server.error("Access to .git folders is forbidden", 403)
            return True
        for name, (res_path, can_read) in self.reserved_paths.items():
            if (res_path == req_path or res_path in req_path.parents) and (
    ...
    async def _handle_filelist_request(
        self,
    ...
    async def _handle_metadata_request(self, web_request: WebRequest) -> dict[str, Any]:
        requested_file: str = web_request.get_str("filename")
        metadata: dict[str, Any] | None
        metadata = self.gcode_metadata.get(requested_file, None)
        if metadata is None:
            raise self.server.error(
                f"Metadata not available for <{requested_file}>",
                404,
            )
        metadata["filename"] = requested_file
        return metadata

    async def _handle_metascan_request(self, web_request: WebRequest) -> dict[str, Any]:
        async with self.sync_lock:
            ...
            if metadata is None:
                raise self.server.error(
                    f"Failed to parse metadata for file '{requested_file}'",
                    500,
                )
            metadata["filename"] = requested_file
            return metadata
    ...
    async def _handle_list_roots(self, web_request: WebRequest) -> list[dict[str, Any]]:
        ...
        for name, path in self.file_paths.items():
            ...
            root_list.append({"name": name, "path": path, "permissions": perms})
        return root_list
        ...
        async with self.sync_lock:
            ...
            if req_type == RequestType.POST and root in self.full_access_roots:
                ...
                if directory.strip("/") == root:
                    raise self.server.error("Cannot delete root directory")
                if not os.path.isdir(dir_path):
                    raise self.server.error(f"Directory does not exist ({directory})")
                ...
                if force:
                    ...
                    try:
                        await self.event_loop.run_in_thread(shutil.rmtree, dir_path)
                    except Exception:
                        raise
                else:
                    try:
    ...
    def _convert_request_path(self, request_path: str) -> tuple[str, str]:
        ...
        if len(parts) > 1:
            ...
            if not dest_path.startswith(root_path):
                raise self.server.error(
                    f"Invalid path request, '{request_path}'' is outside root '{root}'",
                )
        return root, dest_path

    async def _handle_file_move_copy(self, web_request: WebRequest) -> dict[str, Any]:
        ...
        if dest_root not in self.full_access_roots:
            raise self.server.error(f"Destination path is read-only: {dest_root}")
        ...
        async with self.sync_lock:
            ...
            if ep == "/server/files/move":
                ...
                if os.path.isdir(source_path):
                    ...
                    if os.path.isfile(dest_path) or os.path.isfile(
                        os.path.join(dest_path, source_base),
                    ):
                        action = "modify_file"
    ...
    async def _handle_zip_files(self, web_request: WebRequest) -> dict[str, Any]:
        async with self.sync_lock:
            store_only = web_request.get_boolean("store_only", False)
            suffix = time.strftime("%Y%m%d-%H%M%S", time.localtime())
            dest: str = web_request.get_str("dest", f"config/collection-{suffix}.zip")
            dest_root, dest_str_path = self._convert_request_path(dest)
            if dest_root not in self.full_access_roots:
                raise self.server.error(f"Destination Root '{dest_root}' is read-only")
            ...
            return {"destination": ret["item"], "action": "zip_files"}
        ...
        with zipfile.ZipFile(str(temp_dest), "w", compression=cptype) as zf:
            for item in item_list:
                ...
                if not item_path.exists():
                    raise self.server.error(f"No file/directory exits at '{item}'")
                ...
                if not item_path.is_dir():
                    raise self.server.error(
                        f"Item at path '{item}' is not a valid file or directory",
                    )
    ...
    def _list_directory(
        ...
        if not os.path.isdir(path):
            raise self.server.error(f"Directory does not exist ({path})")
        ...
        for fname in os.listdir(path):
            ...
            if os.path.isdir(full_path):
                ...
                if root == "gcodes" and ext in VALID_GCODE_EXTS and is_extended:
                    rel_path = self.get_relative_path(root, full_path)
                    metadata: dict[str, Any] = self.gcode_metadata.get(rel_path, {})
    ...
    def get_path_info(
        ...
        if ".git" in real_path.parts:
            ...
            permissions = "rw"
            if root not in self.full_access_roots or (
    ...
    def gen_temp_upload_path(self) -> str:
        ...
        return os.path.join(tempfile.gettempdir(), f"moonraker.upload-{loop_time}.mru")
    ...
    def _parse_upload_args(self, upload_args: dict[str, Any]) -> dict[str, Any]:
        ...
        if not dest_path.startswith(root_path):
            raise self.server.error(f"Cannot write to path: {dest_path}")
        ...
        if (
            ...
            and not os.access(dest_path, os.W_OK, effective_ids=True)
        ):
            logging.info(
    ...
    async def _finish_gcode_upload(self, upload_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if e.status_code == 403:
                raise self.server.error("File is loaded, upload not permitted", 403)
        finfo = await self._process_uploaded_file(upload_info)
        await self.gcode_metadata.parse_metadata(upload_info["filename"], finfo).wait()
        started: bool = False
        queued: bool = False
    ...
    def list_dir(
        ...
        root = parts[0]
        if root not in self.file_paths:
            raise self.server.error(f"Invalid Directory Request: {directory}")
        ...
        if not os.path.isdir(dir_path):
            raise self.server.error(f"Directory does not exist ({dir_path})")
    ...
    async def _handle_file_delete(self, web_request: WebRequest) -> dict[str, Any]:
    ...
    async def delete_file(self, path: str) -> dict[str, Any]:
        async with self.sync_lock:
            ...
            if root not in self.full_access_roots:
                raise self.server.error(f"Path not available for DELETE: {path}", 405)
    ...
    def _sched_changed_event(
        ...
        notify_info: dict[str, Any] = {"action": action, "item": path_info}
        ...
        delay = 0.005 if immediate else 1.0
...
class NotifySyncLock(asyncio.Lock):
    ...
    async def wait_inotify_event(self, current_path: StrOrPath) -> None:
        ...
        timeout = 1200.0 if has_pending else 1.0
        for _ in range(5):
            try:
    ...
    def add_pending_path(self, action: str, pending_path: StrOrPath) -> None:
        ...
        if not self.check_pending or self.dest_path is None or action != self.action:
            return
...
class BaseFileSystemObserver:
    ...
    def parse_gcode_metadata(self, file_path: str) -> asyncio.Event:
        ...
        try:
            ...
            path_info = {}
        if ext not in VALID_GCODE_EXTS or path_info.get("size", 0) == 0:
            evt = asyncio.Event()
            evt.set()
            return evt
        if ext == ".ufp":
    ...
    def _scan_directory_metadata(self, start_path: pathlib.Path) -> Awaitable | None:
        # Use os.walk find files in sd path and subdirs
        ...
        for parent, dirs, files in os.walk(start_path, followlinks=True):
            ...
            for fname in files:
                ...
                if not file_path.is_file() or file_path.suffix not in VALID_GCODE_EXTS:
                    continue
    ...
    def on_item_move(
        ...
        if ret is False:
            ...
            if dest_path.is_file() and dest_path.suffix in VALID_GCODE_EXTS:
...
INOTIFY_BUNDLE_TIME = 0.25
INOTIFY_MOVE_TIME = 1.0
...
class InotifyNode:
    ...
    async def _finish_create_node(self) -> None:
        ...
        self.iobsvr.notify_filelist_changed("create_dir", root, node_path)
    ...
    def _finish_delete_child(self) -> None:
        ...
        for name, is_node in self.pending_deleted_children:
            ...
            self.iobsvr.notify_filelist_changed(f"delete_{item_type}", root, item_path)
        self.pending_deleted_children.clear()
    ...
    def complete_file_write(self, file_name: str) -> None:
        ...
        if self.is_processing():
            logging.debug(
                f"Metadata is processing, suppressing write event: {file_name}",
            )
            return
        pending_node = self.search_pending_event("create_node")
        if pending_node is not None:
    ...
    def schedule_child_delete(self, child_name: str, is_node: bool) -> None:
        if is_node:
            ...
            if child_node is None:
                return
            self.iobsvr.remove_watch(child_node.watch_desc, need_low_level_rm=False)
    ...
    def find_pending_node(self) -> InotifyNode | None:
        if self.is_processing_metadata or "create_node" in self.pending_node_events:
            return self
...
class InotifyObserver(BaseFileSystemObserver):
    def __init__(
        ...
        self.event_loop.add_reader(self.inotify.fileno(), self._handle_inotify_read)
        self.watched_roots: dict[str, InotifyRootNode] = {}
        self.watched_nodes: dict[int, InotifyNode] = {}
        self.pending_moves: dict[int, tuple[InotifyNode, str, asyncio.Handle]] = {}
    ...
    async def _notify_root_updated(
        self,
    ...
    def log_nodes(self) -> None:
        if self.server.is_verbose_enabled():
            ...
            for wdesc, node in self.watched_nodes.items():
                wdir = node.get_path()
                wroot = node.get_root()
                debug_msg += f"\nRoot: {wroot}, Directory: {wdir},  Watch: {wdesc}"
    ...
    def _process_dir_event(self, evt: InotifyEvent, node: InotifyNode) -> None:
        ...
        if evt.mask & iFlags.CREATE:
            ...
            elif self.file_manager.check_reserved_path(full_path, True, False):
                logging.debug(
    ...
    def _process_file_event(self, evt: InotifyEvent, node: InotifyNode) -> None:
        ...
        if evt.mask & iFlags.CREATE:
            logging.debug(f"Inotify file create: {root}, {node_path}, {evt.name}")
            ...
            logging.debug(f"Inotify file delete: {root}, {node_path}, {evt.name}")
            if root == "gcodes" and ext == ".ufp":
                # Don't notify deleted ufp files
                return
            node.schedule_child_delete(evt.name, False)
        elif evt.mask & iFlags.MOVED_FROM:
            logging.debug(f"Inotify file move from: {root}, {node_path}, {evt.name}")
            self._schedule_pending_move(evt, node, False)
        elif evt.mask & iFlags.MOVED_TO:
            logging.debug(f"Inotify file move to: {root}, {node_path}, {evt.name}")
            node.flush_delete()
            moved_evt = self.pending_moves.pop(evt.cookie, None)
    ...
    def need_create_notify(self, file_path: str) -> bool:
        ...
        rel_path = self.file_manager.get_relative_path("gcodes", file_path)
        if (
            self.gcode_metadata.is_file_processing(rel_path)
            and rel_path in self.processing_gcode_files
        ):
            logging.debug(
    ...
    def notify_filelist_changed(
        ...
        if source_path is not None and source_root is not None:
            src_rel_path = self.file_manager.get_relative_path(source_root, source_path)
            result["source_item"] = {"path": src_rel_path, "root": source_root}
        key = f"{action}-{root}-{rel_path}"
        self.event_loop.create_task(self._finish_notify(result, sync_fut, key))
...
class MetadataStorage:
    def __init__(self, config: ConfigHelper, db: DBComp) -> None:
        self.server = config.get_server()
        self.enable_object_proc = config.getboolean("enable_object_processing", False)
        self.default_metadata_parser_timeout = config.getfloat(
            "default_metadata_parser_timeout",
            20.0,
        )
        self.gc_path = ""
        db.register_local_namespace(METADATA_NAMESPACE)
        self.mddb = db.wrap_namespace(METADATA_NAMESPACE, parse_keys=False)
        version = db.get_item("moonraker", "file_manager.metadata_version", 0).result()
        if version != METADATA_VERSION:
            # Clear existing metadata when version is bumped
            self.mddb.clear()
            db.insert_item(
                "moonraker",
                "file_manager.metadata_version",
                METADATA_VERSION,
            )
        ...
        self.pending_requests: dict[str, tuple[dict[str, Any], asyncio.Event]] = {}
    ...
    def _has_valid_data(self, fname: str, path_info: dict[str, Any]) -> bool:
        if path_info.get("ufp_path") is not None:
            # UFP files always need processing
            return False
        mdata: dict[str, Any]
        mdata = self.metadata.get(fname, {"size": "", "modified": 0})
        for field in ["size", "modified"]:
            if mdata[field] != path_info.get(field):
                return False
        return True
            ...
            if "thumbnails" in metadata:
                ...
                for thumb in metadata["thumbnails"]:
                    ...
                    try:
                        ...
                        await eventloop.run_in_thread(shutil.move, thumb_path, new_path)
    ...
    def parse_metadata(self, fname: str, path_info: dict[str, Any]) -> asyncio.Event:
        ...
        if ext not in VALID_GCODE_EXTS or self._has_valid_data(fname, path_info):
    ...
    async def _process_metadata_update(self) -> None:
        while self.pending_requests:
            fname, (path_info, mevt) = list(self.pending_requests.items())[0]
            ...
            while retries:
                try:
                    ...
                    await self.server.send_event(
                        "file_manager:metadata_processed",
                        fname,
                    )
                    break
            else:
                if ufp_path is None:
                    ...
                    self.mddb[fname] = self.metadata[fname]
                logging.info(f"Unable to extract metadata from file: {fname}")
            self.pending_requests.pop(fname, None)
            mevt.set()
        self.busy = False

    async def _run_extract_metadata(self, filename: str, ufp_path: str | None) -> None:
