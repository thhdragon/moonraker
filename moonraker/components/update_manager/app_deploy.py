...
class AppDeploy(BaseDeploy):
    def __init__(self, config: ConfigHelper, prefix: str) -> None:
        ...
        self.pip_ver_date: datetime = datetime.fromtimestamp(0.0, UTC)
    ...
    def _configure_path(self, config: ConfigHelper, reserve: bool = True) -> None:
        ...
        if (
            reserve
            and self.name not in ["moonraker", "klipper"]
    ...
    def _configure_managed_services(self, config: ConfigHelper) -> None:
        ...
        self.log_debug(f"Managed services: {self.managed_services}")
    ...
    def check_same_paths(
        ...
        if self.py_exec is None:
            return False
        try:
            return self.path.samefile(app_path) and self.py_exec.samefile(executable)
        except Exception:
            return False
    ...
    async def _read_python_reqs(self) -> list[str]:
        ...
        return await eventloop.run_in_thread(
            pip_utils.read_requirements_file,
            self.python_reqs,
        )
    ...
    async def _get_file_hash(self, filename: pathlib.Path | None) -> str | None:
