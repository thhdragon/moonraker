...
class NetDeploy(AppDeploy):
    ...
    def _configure_persistent_files(self, config: ConfigHelper) -> None:
        ...
        if (
            self.type == AppType.ZIP
            and self.virtualenv is not None
            and self.virtualenv in self.path.parents
        ):
        ...
        if not self._path_writable:
            ...
            if rinfo.is_file():
                try:
                    ...
                    self.asset_name = uinfo.get("asset_name")
                except Exception:
    ...
    async def _detect_fallback(self) -> bool:
        ...
        fallback_defs = {"mainsail": "mainsail-crew", "fluidd": "fluidd-core"}
    ...
    async def initialize(self) -> dict[str, Any]:
        ...
        self.rollback_repo = storage.get(
            "rollback_repo",
            self.repo if self._is_valid else "?",
        )
    ...
    async def _finalize_executable(self, tmp_file: pathlib.Path, new_ver: str) -> None:
        ...
        dest = await eventloop.run_in_thread(shutil.move, str(tmp_file), str(self.path))
    ...
    async def update(
        ...
        if rollback_info is not None:
            ...
            if self.remote_version == "?":
                await self._get_remote_version()
                if self.remote_version == "?":
                    raise self.server.error(f"{self.prefix}Unable to locate update")
