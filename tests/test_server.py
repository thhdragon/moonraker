...
async def test_unparsed_config_items(full_server: Server):
    expected_warnings = [
        ...
        "Unparsed config option 'unknown_option: True' detected in section [server].",
    ]
    warn_cnt = 0
...
@pytest.mark.asyncio
async def test_file_logger(base_server: Server, path_args: dict[str, pathlib.Path]):
    log_path = path_args.get("moonraker.log", None)
    assert log_path is not None and log_path.exists()


def test_signal_handler(base_server: Server, event_loop: asyncio.AbstractEventLoop):
...
class TestInstantiation:
    ...
    def test_app_args(self, path_args: dict[str, pathlib.Path], base_server: Server):
        args = {
            "config_file": str(path_args["moonraker.conf"]),
    ...
    async def test_notification(self, base_server: Server):
        ...
        ret = await fut
        expected = {"jsonrpc": "2.0", "method": "notify_test_event", "params": ["test"]}
        assert expected == ret
...
class TestLoadComponent:
    def test_load_component_fail(self, base_server: Server):
        with pytest.raises(ServerError):
            base_server.load_component(base_server.config, "invalid_component")
    ...
    def test_load_component_fail_with_default(self, base_server: Server):
        comp = base_server.load_component(base_server.config, "invalid_component", None)
        assert comp is None
...
class TestCoreServer:
    ...
    def test_http_servers(self, core_server: Server):
        ...
        assert app.http_server is None and app.secure_server is None
    ...
    def test_register_method_exists(self, core_server: Server):
        with pytest.raises(ServerError):
            core_server.register_remote_method("shutdown_machine", lambda: None)
...
class TestServerInit:
    ...
    def test_config_backup(
        self, full_server: Server, path_args: dict[str, pathlib.Path]
    ):
...
class TestServerStart:
    ...
    def test_http_servers(self, server: Server):
        ...
        assert app.http_server is not None and app.secure_server is None
...
@pytest.mark.asyncio
async def test_component_exit_error(
    base_server: Server, caplog: pytest.LogCaptureFixture
):
...
@pytest.mark.asyncio
async def test_component_close_error(
    base_server: Server, caplog: pytest.LogCaptureFixture
):
...
def test_main(
    ...
    def mock_init(self: Server):
        ...
        if tries:
            reason = "restart"
            tries.pop(0)
        self.event_loop.delay_callback(0.01, self._stop_server, reason)
    ...
    assert (
        code == 0
        and "Attempting Server Restart..." in caplog.messages
        and "Server Shutdown" == caplog.messages[-1]
    )


@pytest.mark.run_paths(moonraker_conf="invalid_config.conf")
def test_main_config_error(
    path_args: dict[str, pathlib.Path], caplog: pytest.LogCaptureFixture
):
    cfg_path = path_args["moonraker.conf"]
    args = MockArgs("", True, str(cfg_path))
    try:
...
@pytest.mark.run_paths(
    moonraker_conf="invalid_config.conf", moonraker_bkp=".moonraker.conf.bkp"
)
@pytest.mark.usefixtures("event_loop")
def test_main_restore_config(
    path_args: dict[str, pathlib.Path],
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
):
    ...
    assert (
        code == 0
        and "Loaded server from most recent working configuration:" in caplog.text
    )
...
class TestEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_http_server_info(self, server: Server, http_client: HttpClient):
    ...
    @pytest.mark.asyncio
    async def test_http_server_config(self, server: Server, http_client: HttpClient):
        cfg = server.config.get_parsed_config()
        ret = await http_client.get("/server/config")
        assert ret["result"]["config"] == cfg

    @pytest.mark.asyncio
    async def test_websocket_server_info(
        self, server: Server, websocket_client: WebsocketClient
    ):
    ...
    @pytest.mark.asyncio
    async def test_websocket_server_config(
        self, server: Server, websocket_client: WebsocketClient
    ):
        cfg = server.config.get_parsed_config()
...
def test_server_restart(
    base_server: Server, http_client: HttpClient, event_loop: asyncio.AbstractEventLoop
):
    result = {}
