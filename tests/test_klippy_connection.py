...
async def test_klippy_startup(full_server: Server):
    ...
    ret = await asyncio.wait_for(asyncio.gather(*futs), 4.0)
    assert (
        ret == ["id", "started", "ready"]
...
async def test_klippy_reconnect(ready_server: Server, klippy: KlippyProcess):
    ...
    klippy.restart()
    ret = await asyncio.wait_for(asyncio.gather(*futs), 6.0)
...
async def test_status_update(ready_server: Server, klippy: KlippyProcess):
    ...
    ready_server.register_event_handler("server:status_update", on_status_update)
...
async def test_connection_close(full_server: Server):
    ...
    await asyncio.wait_for(asyncio.gather(ret, ret2), 4.0)
...
@pytest.mark.run_paths(klippy_uds="fake_uds")
@pytest.mark.asyncio
async def test_no_uds_access(base_server: Server, path_args: dict[str, pathlib.Path]):
    attempts = [1, 2, 3]
...
async def test_write_cancelled(base_server: Server):
    ...
    base_server.event_loop.delay_callback(0.01, task.cancel)
    with pytest.raises(asyncio.CancelledError):
        await task


@pytest.mark.asyncio
async def test_read_error(base_server: Server, caplog: pytest.LogCaptureFixture):
...
async def test_read_cancelled(base_server: Server):
    ...
    kconn = base_server.klippy_connection
    task = base_server.event_loop.create_task(kconn._read_stream(mock_reader))
    base_server.event_loop.delay_callback(0.01, task.cancel)
    with pytest.raises(asyncio.CancelledError):
        await task
...
@pytest.mark.asyncio
async def test_read_decode_error(base_server: Server, caplog: pytest.LogCaptureFixture):
...
def test_process_unknown_method(base_server: Server, caplog: pytest.LogCaptureFixture):
...
def test_process_unknown_request(base_server: Server, caplog: pytest.LogCaptureFixture):
    cmd = {"id": 4543}
    kconn = base_server.klippy_connection
    kconn._process_command(cmd)
