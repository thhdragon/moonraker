...
class WebsocketClient:
    ...
    def __init__(self, type: str = "ws", port: int = 7010) -> None:
    ...
    async def connect(self, token: str | None = None) -> None:
        ...
        self.ws = await tornado.websocket.websocket_connect(
            url, connect_timeout=2.0, on_message_callback=self._on_message_received
        )

    async def request(
        self, remote_method: str, args: dict[str, Any] = {}
    ) -> dict[str, Any]:
        if self.ws is None:
    ...
    def _encode_request(self, method: str, args: dict[str, Any]) -> tuple[str, int]:
        ...
        if method is not None:
            if req_id is None:
                ...
                if not isinstance(params, list):
                    pytest.fail(
                        f"jsonrpc notification paramsshould always be a list: {data}"
                    )
            ...
            if "result" in resp:
                ...
                pytest.fail(f"Invalid jsonrpc packet, no result or error: {data}")
        else:
            # Invalid json
    ...
    def close(self):
        for fut in self.pending_requests.values():
            if not fut.done():
                fut.set_exception(WebsocketError(0, "Closing Websocket Client"))
