...
MOONRAKER_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
sys.path.append(os.path.join(MOONRAKER_PATH, "moonraker"))
from utils import hash_directory, retrieve_git_version
...
OWNER_REPOS = {"moonraker": "arksine/moonraker", "klippy": "klipper3d/klipper"}
INSTALL_SCRIPTS = {
    "klippy": {
    ...
    "moonraker": {"debian": "install-moonraker.sh"},
}
...
def search_install_script(data: str, regex: str, exclude: str) -> list[str]:
    items: set[str] = set()
...
def generate_dependency_info(repo_path: str, app_name: str) -> None:
    ...
    req_file_name = os.path.join(repo_path, "scripts", f"{app_name}-requirements.txt")
...
def clean_repo(path: str) -> None:
    ...
    process = subprocess.Popen(
        prog, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=path
    )
...
def get_releases() -> list[dict[str, Any]]:
    ...
    prog = ("curl", "-H", "Accept: application/vnd.github.v3+json", RELEASE_URL)
    process = subprocess.Popen(prog, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    response, err = process.communicate()
    retcode = process.wait()
    if retcode != 0:
        print(
            f"Release list request returned with code {retcode},"
            f" response:\n{err.decode()}"
        )
        return []
...
def get_last_release_info(
    moonraker_version: str, is_beta: bool, releases: list[dict[str, Any]]
) -> dict[str, Any]:
...
def get_commit_log(path: str, release_info: dict[str, Any]) -> list[dict[str, Any]]:
...
def generate_version_info(
    path: str, source_dir: str, channel: str, release_tag: str | None = None
) -> dict[str, Any]:
...
def create_zip(repo_path: str, repo_name: str, output_path: str) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "-o",
        "--output",
        default=os.path.join(MOONRAKER_PATH, ".dist"),
        metavar="<output_path>",
        help="Path to output directory",
    )
    parser.add_argument("-b", "--beta", action="store_true", help="Tag release as beta")
    ...
    try:
        ...
        all_info["klipper"] = generate_version_info(kpath, "klippy", channel, rtag)
        ...
        commit_log["klipper"] = get_commit_log(kpath, last_rinfo.get("klipper", {}))
