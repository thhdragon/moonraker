...
def open_db(db_path: str) -> lmdb.Environment:
    return lmdb.open(db_path, map_size=MAX_DB_SIZE, max_dbs=MAX_NAMESPACES)


def _do_dump(
    namespace: bytes, db: object, backup: TextIO, txn: lmdb.Transaction
) -> None:
    ...
    if expected_key_count != count:
        print(
            "Warning: Key count mismatch for namespace "
...
def backup(args: dict[str, Any]):
    ...
    if expected_ns_cnt != count:
        print(
            "Warning: namespace count mismatch: "
...
def _process_header(key: bytes, value: bytes) -> int:
    if key != HEADER_KEY:
        raise DBToolError(
            f"Database Backup does not contain a valid header key,  got {key.decode()}"
        )
    val_parts = value.split(b"=", 1)
    if val_parts[0] != b"namespace_count":
        raise DBToolError(
            f"Database Backup has an invalid header value, got {value.decode()}"
        )
...
def _process_namespace(key: bytes, value: bytes) -> tuple[bytes, int]:
    ...
    if key_parts[0] != b"namespace":
        raise DBToolError(f"Invalid Namespace Key '{key.decode()}', ID not prefixed")
    namespace = key_parts[1]
    val_parts = value.split(b"=", 1)
    if val_parts[0] != b"entries":
        raise DBToolError(
            f"Invalid Namespace value '{value.decode()}', entry count not present"
        )
...
def _process_line(line: str) -> tuple[bytes, bytes]:
    ...
    if match is None:
        ...
        raise DBToolError(f"Invalid DB Entry match: {line}")
    parts = match.groups()
    if len(parts) != 4:
        raise DBToolError(f"Invalid DB Entry, does not contain all data: {line}")
...
def restore(args: dict[str, Any]):
    ...
    if not dest_path.exists():
        print(
            f"Destination path '{dest_path}' does not exist, directorywill be created"
        )
    ...
    if dest_path.joinpath("data.mdb").exists():
        ...
        print(
            f"Warning: database file at found in '{dest_path}', "
    ...
    if expected_ns_count != namespace_count:
        print(
            "Warning: Namespace count mismatch, expected: "
